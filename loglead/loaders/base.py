...
class GELFLoader(BaseLoader):
    ...
    def load(self):
        print("1")
        with open(self.filename, 'r') as file:
            # Parse JSON lines into a list of dictionaries in one go
            json_dicts = [json.loads(line) for line in file]
            print("read lines:", len(json_dicts))

        # Create a single DataFrame from the list of dictionaries
        self.df = pl.DataFrame(json_dicts)
    ...
    def preprocess(self):
        ...
        #self.df = self.df.with_columns(
        #    (pl.col("anomaly") == 0).alias("normal"),  # Creates a 'normal' column, True where 'anomaly' is 0
        #    (pl.col("anomaly") == 1).alias("anomaly")  # Overwrites 'anomaly' column with True where 'anomaly' is 1
        #)